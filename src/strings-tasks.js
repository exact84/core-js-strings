...
function extractNameFromTemplate(value) {
  return value.trim().substring(value.indexOf(' ') + 1, value.length - 1);
}
...
function encodeToRot13(str) {
  const arr0 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const arrR = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[a-zA-Z]/g, (char) => {
    return arrR[arr0.indexOf(char)];
  });
}
